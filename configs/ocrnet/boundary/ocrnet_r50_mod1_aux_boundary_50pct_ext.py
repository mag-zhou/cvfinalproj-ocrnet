# Mod 1 (auxiliary boundary head), 50% ADE20K --
# CONTINUATION run: load weights from iter_40000.pth of the original 50pct
# run and train another 40k iters with a fresh PolyLR over [0, 40000).
# Final checkpoint at iter_40000 of THIS run = 80k iters of training in total.
#
# This mirrors `ocrnet_r50_mod3_modulated_ocr_50pct_ext.py` exactly, only
# pointing at the Mod 1 work_dir instead.
#
...
#
# To run:
#   sbatch train_slurm_mod1_50pct_ext.sh

_base_ = ['./ocrnet_r50_mod1_aux_boundary_50pct.py']
...
load_from = '/orcd/scratch/orcd/003/janetguo/cvfinalproj-ocrnet/work_dirs/ocrnet_r50_mod1_aux_boundary_50pct/iter_40000.pth'

# Fresh 40k schedule, identical to the original (so it's directly comparable
# to the Mod 3 ext run that did the same warm-restart pattern).
optim_wrapper = dict(
